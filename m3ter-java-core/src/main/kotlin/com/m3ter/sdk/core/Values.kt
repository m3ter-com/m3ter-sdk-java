...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { M3terInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw M3terInvalidDataException("`$name` is not set")
            is JsonNull -> throw M3terInvalidDataException("`$name` is null")
            else -> throw M3terInvalidDataException("`$name` is invalid, received $this")
        }
