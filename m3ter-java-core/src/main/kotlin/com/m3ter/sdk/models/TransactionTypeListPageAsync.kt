...
private constructor(
    ...
    class AutoPager(
        private val firstPage: TransactionTypeListPageAsync,
    ) {

        fun forEach(
            action: Predicate<TransactionType>,
            executor: Executor
        ): CompletableFuture<Void> {
            fun CompletableFuture<Optional<TransactionTypeListPageAsync>>.forEach(
                action: (TransactionType) -> Boolean,
                executor: Executor
            ): CompletableFuture<Void> =
                thenComposeAsync(
                    { page ->
        ...
        fun toList(executor: Executor): CompletableFuture<List<TransactionType>> {
            val values = mutableListOf<TransactionType>()
