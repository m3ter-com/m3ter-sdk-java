...
private constructor(
    ...
    class AutoPager(
        private val firstPage: CounterPricingListPageAsync,
    ) {

        fun forEach(
            action: Predicate<CounterPricing>,
            executor: Executor
        ): CompletableFuture<Void> {
            fun CompletableFuture<Optional<CounterPricingListPageAsync>>.forEach(
                action: (CounterPricing) -> Boolean,
                executor: Executor
            ): CompletableFuture<Void> =
                thenComposeAsync(
                    { page ->
        ...
        fun toList(executor: Executor): CompletableFuture<List<CounterPricing>> {
            val values = mutableListOf<CounterPricing>()
